...
namespace libwebstreamer
{
    PipeJoint make_pipe_joint(const std::string &media_type, const std::string &name)
    {
        ...
        // GstPad *sink_bin_pad = gst_ghost_pad_new("sink", GST_INTER_SINK(inter_sink)->sinkpad);
        // // gst_object_unref(sink_pad);
        // gst_pad_set_active(sink_bin_pad, TRUE);
        // gst_element_add_pad(sink_bin, sink_bin_pad);

        g_object_set_data(G_OBJECT(inter_sink), "media-type", (gchar *)media_type.c_str());
        ///-----------------
        PipeJoint pipejoint;
        pipejoint.upstream_joint = inter_sink;
